...
const STATUS_STYLES = {
  '待訂貨': 'bg-secondary-dark text-white border-secondary-dark',
  '已訂購': 'bg-yellow-400 text-white border-yellow-400',
  '已出貨': 'bg-brand text-white border-brand',
}
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col">
        ...
        <div className="px-6 pt-4">
          <div className="flex items-center gap-2">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${step >= 1 ? 'bg-brand text-white' : 'bg-gray-200 text-gray-500'}`}>1</div>
            <div className={`flex-1 h-1 rounded-full ${step >= 2 ? 'bg-brand' : 'bg-gray-200'}`} />
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${step >= 2 ? 'bg-brand text-white' : 'bg-gray-200 text-gray-500'}`}>2</div>
          </div>
        </div>
                  ...
                  {COUNTRIES.map(c => (
                    <button
                      ...
                      className={`flex-1 py-2 px-3 rounded-lg text-sm font-medium border transition-colors duration-150 ${
                        orderData.country === c
                          ? c === '台灣' ? 'bg-yellow-400 text-white border-yellow-400'
                          : c === '韓國' ? 'bg-secondary-dark text-white border-secondary-dark'
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-3 cursor-pointer select-none">
                      <div className="relative flex-shrink-0">
                        ...
                        />
                        <div className={`w-10 h-6 rounded-full transition-colors duration-200 ${orderData.has_proxy_fee ? 'bg-brand' : 'bg-gray-300'}`} />
                    ...
                    <label className="flex items-center gap-3 cursor-pointer select-none">
                      <div className="relative flex-shrink-0">
                        ...
                        />
                        <div className={`w-10 h-6 rounded-full transition-colors duration-200 ${orderData.has_tax ? 'bg-brand' : 'bg-gray-300'}`} />
          ...
          {step === 2 && (
            <>
              {(orderData.has_proxy_fee || orderData.has_tax) && (
                <div className="bg-brand-light border border-brand-light rounded-lg px-4 py-2.5 text-xs text-brand flex flex-wrap gap-2">
                  <span className="font-semibold">已套用：</span>
                  {orderData.has_tax && <span>消費稅 ×1.10</span>}
                  {orderData.has_proxy_fee && <span>代購費 ×1.08</span>}
                  <span className="text-brand">（小計已自動計入）</span>
                </div>
              )}
              ...
              <button
                ...
                className="w-full py-2.5 border-2 border-dashed border-gray-300 rounded-lg text-sm text-gray-500 hover:border-brand hover:text-brand transition-colors duration-150 font-medium"
              >
                + 新增商品
              </button>
